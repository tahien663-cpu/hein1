...
export default function Login() {
    ...
    try {
      ...
      if (isRegister) {
        ...
        if (data.token) {
          ...
          try {
            await apiService.verifyToken();  // Assuming verifyToken exists; if not, remove or replace with getChatHistory as in original
            setTimeout(() => {
              navigate('/home');
            }, 1500);
          } catch (verifyError) {
  ...
  useEffect(() => {
    ...
    if (token) {
      apiService.verifyToken().then(() => navigate('/home')).catch(() => {});
    }
  }, [navigate]);
