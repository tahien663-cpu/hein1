...
export default function Chat() {
  ...
  const loadChat = useCallback(async (id) => {
    try {
      const response = await apiService.request(`/chat/${id}`, { method: 'GET' });
  ...
  }, [isLoading, insertBold, insertItalic, insertCode, handleSendMessage]);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      const data = await retryFetch(() => apiService.sendMessage(currentChatId, input));
      const aiMessage = {
        id: Date.now().toString(),
        role: 'ai',
  ...
  const handleWebSearch = useCallback(async () => {
    if (!input.trim() || isLoading) return;
    setShowActionDropdown(false);
    handleSendMessage({ webSearch: true });
  ...
  const handleGenerateImage = useCallback(async () => {
    if (!input.trim() || isLoading) return;
    setShowActionDropdown(false);
    handleSendMessage({ generateImage: true });
  }, [input, isLoading, handleSendMessage]);
              ...
              <div className={`max-w-[70%] p-4 rounded-2xl relative group ${msg.role === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white'} shadow-sm`}>
                ...
                <div className="flex justify-between items-center mt-2 text-xs opacity-70">
                  ...
                  {msg.role === 'user' && msg.id !== 'welcome' && (
                    <button 
                      onClick={() => deleteMessage(currentChatId, msg.id)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-red-500/20"
                    >
                      <Trash2 className="w-4 h-4 text-red-500" />
                    </button>
