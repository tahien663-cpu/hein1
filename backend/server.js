...
async function webSearch(query) {
  try {
    if (!cheerio) {
      console.error('❌ Cheerio not loaded');
      return [];
    }
...
app.post('/api/register', authLimiter, async (req, res) => {
  try {
    console.log('📝 Register request from:', req.get('origin'));
...
app.delete('/api/message/:messageId', authenticateToken, async (req, res) => {
  try {
    ...
    // Verify message exists and get chat_id
